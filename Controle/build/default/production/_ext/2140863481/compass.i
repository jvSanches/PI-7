...
# 23 "../Lab6_/compass.c"
volatile int16_t Ycorr = 0, Xcorr = 0;

# 29
uint8_t compass_get_id() {
uint8_t command_byte = (5 << 5) + 0;
spi_write(command_byte);
uint8_t compass_id = spi_read();
return compass_id;
}
...
compass_set_datarate(7);
compass_set_operativemode(3);
compass_set_scale(0);
...
for(int i = 1; i < 8; i++){
Xread = (Xread + compass_get_X()) / 2;
Yread = (Yread + compass_get_Y()) / 2;
delay_ms(1);
}
